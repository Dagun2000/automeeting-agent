...
대상은 **장소·풍경·건축물 "그리고" 유물·소품 같은 사물**을 포함한 환경
요소로 한정합니다(사물도 명확히 포함됩니다 - 장소로 좁혀 읽지 마세요).
**인물/캐릭터 초상은 절대 후보에서 제외**하세요(캐릭터가 아무리 자세히
논의됐어도 이미지 대상이 될 수 없습니다). 아래 두 조건을 **모두** 충족하는
대상만 image_tasks에 포함하세요:
1. 구체적 대상(장소 또는 사물/유물)이 최소 1개 이상 언급됨
2. 그 대상에 대한 시각적 특징(형태·재질·색감·분위기 등 무엇이든) 묘사가
   함께 언급됨 - 명시적 톤 단어("다크판타지" 등)가 아니어도, 대상 묘사
   자체에 시각 정보가 담겨 있으면 충족. **짧은 상태 묘사 단어 하나(예:
   "무너진", "부서진", "그을린", "부러진")만으로도 이 조건은 충족됩니다** -
   "돌담이 무너진 폐허"나 "부러진 왕관"처럼 형용사 하나짜리 묘사도 시각
   정보로 인정하세요. 여러 문장에 걸친 풍부한 묘사가 있어야만 충족되는 게
   아닙니다.
...
**사건 목록도 brief에 포함하세요.** 엔티티 다섯 카테고리와는 별도로,
회의에서 과거에 실제로 일어난 일로 명시적으로 서술된 구체적 1회성
사건이 있으면 brief에 "사건 목록"으로 나열하세요("~을 구체화할 것" 같은
지시가 아니라, 엔티티 항목처럼 이미 확정된 목록으로). 판단 기준:
- 조건문("~하면 ~한다")으로만 언급되고 실제 발생 여부가 불확실한 것은
  사건이 아닙니다.
- 반복되는 게임플레이 행동(탐험/전투/선택 등)은 1회성 사건이 아닙니다.
- 인물의 상태·역할·배경을 나타내는 형용사적 묘사(예: "몰락한 왕국의
  기사")는 사건이 아닙니다 - "왕국이 몰락했다"처럼 서술된 경우만 사건입니다.
- 위 조건을 만족하는 사건이 하나도 없으면 "사건 목록: 없음"이라고
  명시하세요(빈 목록도 확정된 판단입니다 - Worker가 다시 찾게 하지
  마세요).
